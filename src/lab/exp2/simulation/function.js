window.view = {
	...
	validateRectangleInputs: function() {
		...
		if ( a !== '2') //CHANGES MADE
			alert('Incorrect value of input variables(arguments). Calculating the area of a right angle triangle requires the length of the two different  side of the right angle triangle. Try again.')
		else if ( b !== 'float' )
			alert('Incorrect datatype of input variables(arguments). The value of the sides of a right angletriangle need not be integers. Try again.')
		else if ( c !== 'float' )
	...
	displayFunctionForRectangle: function() {
		document.getElementById('functionRectangle').innerHTML += 'float area_rightangletriangle (float a,float b)<br>{<br> &emsp; float area = 0.5*a*b;<br> &emsp; return area;<br>}'
	},
